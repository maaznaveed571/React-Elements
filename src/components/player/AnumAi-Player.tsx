...
client.addListener(AnamEvent.SESSION_READY, () => {
    ...
    useEffect(() => {
        if (anamClientRef.current) {
            anamClientRef.current.addListener(AnamEvent.MESSAGE_STREAM_EVENT_RECEIVED, (event) => {
                console.log(event, "event")
                setTranscript((prev) => `${prev ? prev : ""} ${event.content}`);
            });
            anamClientRef.current.addListener(AnamEvent.INPUT_AUDIO_STREAM_STARTED, (event) => {
                console.log(event,"event")
                if (event) {
                    setIsListening(true);
                } else {
