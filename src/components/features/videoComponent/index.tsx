...
export default function AssessmentPlayer() {
  ...
  const preloadVideo = async (i: number) => {
  ...
  // Preload first + second when mounted
  useEffect(() => {
    preloadVideo(0); // preload first
    preloadVideo(1); // preload next
  }, []);

  // Always preload the next one in background
  useEffect(() => {
  ...
  const currentSrc = blobs[currentIndex] || questions[currentIndex].video;
